...
export default function BalanceCard({ monthKey }: Props) {
  const { totalIncome, totalExpenses, investments, available } = useFinanceSummary(monthKey);
  const label = monthKey ? formatMonthLabel(monthKey) : new Date().toLocaleDateString("pt-BR", { month: "long", year: "numeric" });
  ...
  return (
    <div className="mx-5 mt-5 gradient-primary rounded-2xl p-6 relative overflow-hidden border border-primary/20">
      ...
      <div className="relative z-10">
        <p className="text-xs text-foreground/60 uppercase tracking-widest mb-1.5">Saldo do mês</p>
        ...
        <div className="flex gap-3 mt-5 flex-wrap">
          ...
          <div className="flex-1 min-w-[70px]">
            ...
            <p className="text-[13px] font-semibold text-accent capitalize">{label}</p>
          </div>
        </div>
      </div>
    </div>
